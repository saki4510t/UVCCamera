...
//		setRetainInstance(true);
	...
	public void onDestroyView() {
		if (DEBUG) Log.w(TAG, "onDestroyView:");
		super.onDestroyView();
	}
	...
	private final OnDeviceConnectListener mOnDeviceConnectListener = new OnDeviceConnectListener() {
		...
		public void onAttach(final UsbDevice device) {
			if (DEBUG) Log.w(TAG, "OnDeviceConnectListener#onAttach:");
		...
		public void onConnect(final UsbDevice device, final UsbControlBlock ctrlBlock, final boolean createNew) {
			if (DEBUG) Log.w(TAG, "OnDeviceConnectListener#onConnect:");
		}

		@Override
		public void onDisconnect(final UsbDevice device, final UsbControlBlock ctrlBlock) {
			if (DEBUG) Log.w(TAG, "OnDeviceConnectListener#onDisconnect:");
		}

		@Override
		public void onDettach(final UsbDevice device) {
			if (DEBUG) Log.w(TAG, "OnDeviceConnectListener#onDettach:");
		...
		public void onCancel(final UsbDevice device) {
			if (DEBUG) Log.w(TAG, "OnDeviceConnectListener#onCancel:");
			enableButtons(false);
		}
	};
	...
	private void tryOpenUVCCamera(final boolean requestPermission) {
		if (DEBUG) Log.w(TAG, "tryOpenUVCCamera:");
		openUVCCamera(0);
	}

	private void openUVCCamera(final int index) {
		if (DEBUG) Log.w(TAG, "openUVCCamera:index=" + index);
	...
	private final OnClickListener mOnClickListener = new OnClickListener() {
		...
		public void onClick(final View v) {
			...
			case R.id.start_button:
				if (DEBUG) Log.w(TAG, "onClick:start");
				// start service
				final List<UsbDevice> list = mUSBMonitor.getDeviceList();
				if (list.size() > 0) {
					if (mCameraClient == null)
			...
			case R.id.stop_button:
				if (DEBUG) Log.w(TAG, "onClick:stop");
				// stop service
				if (mCameraClient != null) {
					mCameraClient.disconnect();
					mCameraClient.release();
			...
			case R.id.camera_view_sub:
				if (DEBUG) Log.w(TAG, "onClick:sub view");
				if (isSubView) {
					mCameraClient.removeSurface(mCameraViewSub.getHolder().getSurface());
				} else {
					mCameraClient.addSurface(mCameraViewSub.getHolder().getSurface(), false);
			...
			case R.id.record_button:
				if (DEBUG) Log.w(TAG, "onClick:record");
			...
			case R.id.still_button:
				if (DEBUG) Log.w(TAG, "onClick:still capture");
	...
	private final OnCheckedChangeListener mOnCheckedChangeListener = new OnCheckedChangeListener() {
		...
		public void onCheckedChanged(final CompoundButton buttonView, final boolean isChecked) {
			if (DEBUG) Log.w(TAG, "onCheckedChanged:" + isChecked);
			if (isChecked) {
				mCameraClient.addSurface(mCameraView.getSurface(), false);
//				mCameraClient.addSurface(mCameraViewSub.getHolder().getSurface(), false);
			} else {
