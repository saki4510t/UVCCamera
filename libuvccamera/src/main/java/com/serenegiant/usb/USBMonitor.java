...
public final class USBMonitor {
	...
	public USBMonitor(final Context context, final OnDeviceConnectListener listener) {
		if (DEBUG) Log.w(TAG, "USBMonitor:Constructor");
		if (listener == null)
		...
		destroyed = false;
		if (DEBUG) Log.w(TAG, "USBMonitor:mUsbManager=" + mUsbManager);
	}
		...
		if (list != null) {
			...
			if (keys != null && keys.size() > 0) {
				...
				for (final String key: keys) {
					...
					Log.w(TAG, "key=" + key + ":" + device + ":" + sb.toString());
				}
			} else {
				Log.w(TAG, "no device");
			}
...
//		if (DEBUG) Log.w(TAG, "requestPermission:device=" + device);
	...
	private final void processConnect(final UsbDevice device) {
		...
		mAsyncHandler.post(new Runnable() {
			@Override
			public void run() {
				if (DEBUG) Log.w(TAG, "processConnect:device=" + device);
				UsbControlBlock ctrlBlock;
	...
	private final void processCancel(final UsbDevice device) {
		if (destroyed) return;
		if (DEBUG) Log.w(TAG, "processCancel:");
	...
	private final void processAttach(final UsbDevice device) {
		if (destroyed) return;
		if (DEBUG) Log.w(TAG, "processAttach:");
		if (mOnDeviceConnectListener != null) {
			mAsyncHandler.post(new Runnable() {
				@Override
				public void run() {
	...
	private final void processDettach(final UsbDevice device) {
		if (destroyed) return;
		if (DEBUG) Log.w(TAG, "processDettach:");
		if (mOnDeviceConnectListener != null) {
			mAsyncHandler.post(new Runnable() {
				@Override
				public void run() {
...
//		if (DEBUG) Log.w(TAG, "getDeviceKeyName:" + sb.toString());
		return sb.toString();
	}
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			if (DEBUG) Log.w(TAG, "UsbControlBlock:constructor");
...
//			if (DEBUG) {
				if (mConnection != null) {
					...
					Log.w(TAG, String.format(Locale.US, "name=%s,desc=%d,busnum=%d,devnum=%d,rawDesc=", name, desc, busnum, devnum) + rawDesc);
				} else {
...
//			}
		...
		public synchronized void close() {
			if (DEBUG) Log.w(TAG, "UsbControlBlock#close:");
